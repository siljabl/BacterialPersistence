...
path_single = "../data/low_resolution/optimal_"
path_two = "../data/competition_two_species/optimal_"

# importing data 
lag_Tab = abs(np.loadtxt(f'{path_single}lag-T0{T0}.txt')[:100]     - np.loadtxt(f'{path_two}lag-T0{T0}.txt'))
del_Tab = abs(np.loadtxt(f'{path_single}delta-T0{T0}.txt')[:100]   - np.loadtxt(f'{path_two}delta-T0{T0}.txt'))
lag_T0  = abs(np.loadtxt(f'{path_single}lag-Tab{Tab}.txt')[:100]   - np.loadtxt(f'{path_two}lag-Tab{Tab}.txt'))
del_T0  = abs(np.loadtxt(f'{path_single}delta-Tab{Tab}.txt')[:100] - np.loadtxt(f'{path_two}delta-Tab{Tab}.txt'))
...
im00 = ax[0,0].imshow(lag_T0 / T_T0,   cmap=lag_cmap, origin="lower", vmin=0, aspect="auto", extent=[0, 12, 0, 1]) 
im10 = ax[1,0].imshow(del_T0,          cmap=del_cmap, origin="lower", vmin=0, aspect="auto", extent=[0, 12, 0, 1])
im01 = ax[0,1].imshow(lag_Tab / T_Tab, cmap=lag_cmap, origin="lower", vmin=0, aspect="auto", extent=[0, 24, 0, 1])
im11 = ax[1,1].imshow(del_Tab,         cmap=del_cmap, origin="lower", vmin=0, aspect="auto", extent=[0, 24, 0, 1])
...
fig.savefig(f"compare.png") #, dpi=100)
