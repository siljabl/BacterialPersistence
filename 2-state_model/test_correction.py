...
# importing data 
dir_1 = 'half_dilution_ratio'
dir_2 = 'high_resolution'
lag_Tab = np.loadtxt(f'data/{dir_1}/optimal_lag-T0{T0}.txt') - np.loadtxt(f'data/{dir_2}/optimal_lag-T0{T0}.txt')
del_Tab = np.loadtxt(f'data/{dir_1}/optimal_delta-T0{T0}.txt') - np.loadtxt(f'data/{dir_2}/optimal_delta-T0{T0}.txt')
lag_T0  = np.loadtxt(f'data/{dir_1}/optimal_lag-Tab{Tab}.txt') - np.loadtxt(f'data/{dir_2}/optimal_lag-Tab{Tab}.txt')
del_T0  = np.loadtxt(f'data/{dir_1}/optimal_delta-Tab{Tab}.txt') - np.loadtxt(f'data/{dir_2}/optimal_delta-Tab{Tab}.txt')
...
im00 = ax[0,0].imshow(lag_T0 / T_T0,   cmap=lag_cmap, origin="lower", aspect="auto", extent=[0, 12, 0, 1]) 
im10 = ax[1,0].imshow(del_T0,          cmap=del_cmap, origin="lower", aspect="auto", extent=[0, 12, 0, 1])
im01 = ax[0,1].imshow(lag_Tab / T_Tab, cmap=lag_cmap, origin="lower", aspect="auto", extent=[0, 24, 0, 1])
im11 = ax[1,1].imshow(del_Tab,         cmap=del_cmap, origin="lower", aspect="auto", extent=[0, 24, 0, 1])
...
fig.savefig(f"figs/2state_heatmap_test.png") #, dpi=100)
