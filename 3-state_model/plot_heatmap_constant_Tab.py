...
#importing data 
λd_opt = np.loadtxt(f'{folder}/optimal_λd-Tab_{Tab}.txt')
λr_opt = np.loadtxt(f'{folder}/optimal_λr-Tab_{Tab}.txt')
δ_opt  = np.loadtxt(f'{folder}/optimal_δ-Tab_{Tab}.txt')
...
im0 = ax[0].imshow(λd_opt / T, origin="lower", cmap=d_cmap, aspect="auto", vmin=0, vmax=1.1, extent=[T0_min, T0_max, 0, 1])
im1 = ax[1].imshow(λr_opt / T, origin="lower", cmap=r_cmap, aspect="auto", vmin=0, vmax=1.1, extent=[T0_min, T0_max, 0, 1])
...
# saving
fig.tight_layout()
fig.savefig(f"figs/single_optimal/optimal_heatmap_Tab_{Tab}.png", dpi=100)