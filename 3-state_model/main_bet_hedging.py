...
###########################
p_arr   = np.linspace(0, p_max, ab_res)       # probability of antibiotics
...
λd = np.outer(np.ones(bac_res), np.outer(np.ones(bac_res), np.outer(np.ones(bac_res), λ_arr))).reshape(bac_res, bac_res, bac_res, bac_res) - λ_min / 1000
λr = np.outer(np.ones(bac_res), np.outer(np.outer(np.ones(bac_res), λ_arr), np.ones(bac_res))).reshape(bac_res, bac_res, bac_res, bac_res) - λ_min / 1000
δ  = np.outer(np.ones(bac_res), np.outer(np.outer(δ_arr, np.ones(bac_res)), np.ones(bac_res))).reshape(bac_res, bac_res, bac_res, bac_res)
x  = np.outer(np.outer(np.outer(x_arr, np.ones(bac_res)), np.ones(bac_res)), np.ones(bac_res)).reshape(bac_res, bac_res, bac_res, bac_res)
...
bac_params = {'λd':λd, 'λr':λr, 'δ':δ, 'x':x}
...
# np.savetxt(f"data/optimal_λd-T0_{T0}", λd_opt)
# np.savetxt(f"data/optimal_λr-T0_{T0}", λr_opt)
# np.savetxt(f"data/optimal_δ-T0_{T0}", δ_opt)
